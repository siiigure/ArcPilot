...
function TopicsPage() {
  ...
  const {
    ...
    queryKey: ["tags", "topics", TOPICS_PAGE_SIZE],
    queryFn: ({ pageParam }) =>
      TagsService.listTags({
        limit: TOPICS_PAGE_SIZE,
        afterSlug: pageParam ?? undefined,
      }),
              ...
              >
                <div className="min-w-0">
                  <Link
                    ...
                    className="font-medium text-foreground transition-colors hover:text-[#82ba00]"
                  >
                    {tag.name}
                  </Link>
